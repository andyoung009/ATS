...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids=None):
        """
        Collect the predictions from the current batch and perform on-the-fly
        ...
        Args:
            ...
            clip_ids (tensor): clip indexes of the current batch, dimension is
                N.
        """
        vid_id = clip_ids // self.num_clips
        self.video_labels[vid_id] = labels
        self.video_preds[vid_id] = preds
        ...
        for ind in range(preds.shape[0]):
            ...
            if self.video_labels[vid_id].sum() > 0:
                ...
                ), "self.video_labels[{}].type(torch.FloatTensor) ({}) type is {} " \
                   "while labels[{}].type(torch.FloatTensor) ({}) type is {}".format(
                    vid_id,
                    self.video_labels[vid_id],
                    self.video_labels[vid_id].type(),
                    ind,
                    labels[ind],
                    labels[ind].type()
                )
